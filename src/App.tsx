...
import { AlertTriangle, ArrowRight, Bell, Box, Boxes, CalendarDays, Check, CheckCircle2, ChevronDown, ChevronRight, CircleDollarSign, Clock3, CloudUpload, Download, FileCheck2, Filter, Gauge, IndianRupee, Info, Layers3, PackageCheck, RefreshCcw, Search, ShieldCheck, Sparkles, TrendingDown, TrendingUp, TriangleAlert, Upload, WalletCards, X } from "lucide-react";
...
function App() {
  ...
  useEffect(() => localStorage.setItem(STORAGE_KEY, JSON.stringify({ products, activity })), [products, activity]);
  ...
  function resetDemo() { localStorage.removeItem(STORAGE_KEY); setProducts(demoProducts); setActivity(initialActivity); notify("Demo workspace restored"); }

  return <div className="app-shell">
    <Sidebar page={page} setPage={setPage} open={mobileNav} setOpen={setMobileNav}/>
    <div className="main-column">
      <Topbar page={page} onCopilot={() => setCopilot(true)}/>
      <main>
        {page === "Command center" && <Dashboard insights={insights} activity={activity} setPage={setPage} onApprove={addActivity} openCopilot={()=>setCopilot(true)}/>} 
        {page === "Inventory" && <Inventory insights={insights}/>} 
        ...
        {page === "Data hub" && <DataHub products={products} setProducts={setProducts} onImport={label=>addActivity(label,"imported")} resetDemo={resetDemo}/>} 
      </main>
    </div>
    <Copilot insights={insights} open={copilot} setOpen={setCopilot} onApprove={addActivity}/>
...
function Dashboard({ insights, activity, setPage, onApprove, openCopilot }:{ insights:Insight[]; activity:Activity[]; setPage:(p:Page)=>void; onApprove:(s:string)=>void; openCopilot:()=>void }) {
  ...
  return <>
    <PageHead eyebrow="Monday · 14 September" title="Good morning, Vurimi." copy="Here’s what needs your attention across the Hyderabad depot." actions={<><button className="secondary" onClick={()=>setPage("Data hub")}><Upload size={16}/> Import data</button><button className="primary" onClick={openCopilot}><Sparkles size={16}/> Ask AIDOS</button></>}/>
...
function Inventory({insights}:{insights:Insight[]}) {
