...
function Dashboard({ insights, activity, setPage, onApprove, openCopilot }:{ insights:Insight[]; activity:Activity[]; setPage:(p:Page)=>void; onApprove:(s:string)=>void; openCopilot:()=>void }) {
  ...
  return <>
    <PageHead eyebrow="Monday · 14 September" title="Good morning, Arjun." copy="Here’s what needs your attention across the Hyderabad depot." actions={<><button className="secondary" onClick={()=>setPage("Data hub")}><Upload size={16}/> Import data</button><button className="primary" onClick={openCopilot}><Sparkles size={16}/> Ask AIDOS</button></>}/>
